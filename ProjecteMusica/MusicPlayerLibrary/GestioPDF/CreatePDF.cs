    ...
    public class CreatePDF
    {
        ...
        public static void CreatePDFWithJsonList(string rutaPdf, string jsonList)
        {
            try
            {
                        ...
                        using (Document document = new Document(pdfDocument))
                        {
                            ...
                            foreach (var property in jsonData)
                            {
                                ...
                                if (propertyValue is List<object>)
                                {
                                    ...
                                    {
                                        // Mostrar cada propiedad del objeto de la lista
                ...
                Console.WriteLine("PDF creado correctamente");
            }
            catch (Exception e)
            {
            ...
            string Certpass, string CertificateRoute)
        {
            try
            {
                string rutaPDFTemporal = Path.GetTempFileName() + ".pdf";
                CreatePDFWithJsonList(rutaPDFTemporal, jsonList);
                Sign sign = new Sign();
                sign.CreateSignPDF(CertificateRoute, Certpass, rutaOutputPDFSignado, rutaPDFTemporal);
